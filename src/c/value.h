...
#define ENUM_SPECIAL_HEAP_OBJECT_FAMILIES(F)                                   \
  F(Species,                 species,                   X, _, (_, _, _, X, _, _, _, _, _, _), 70)

// Enumerates the compact object species.
#define ENUM_OTHER_HEAP_OBJECT_FAMILIES(F)                                     \
  F(Key,                     key,                       X, X, (X, _, X, _, _, _, _, _, _, _),  0)\
  ...
  F(Ambience,                ambience,                  _, _, (_, _, _, X, _, _, _, _, _, _), 65)\
  F(ArgumentAst,             argument_ast,              X, X, (_, _, X, _, _, _, _, _, _, _),  9)\
  F(ArgumentMapTrie,         argument_map_trie,         X, _, (_, _, _, _, _, _, X, _, _, _),  7)\
  F(Array,                   array,                     X, X, (_, X, _, X, _, _, _, _, _, X), 11)\
  F(ArrayAst,                array_ast,                 X, X, (_, _, X, _, _, X, _, _, _, _), 10)\
  F(ArrayBuffer,             array_buffer,              X, X, (_, _, _, _, _, _, X, _, _, _), 34)\
  ...
  F(BuiltinMarker,           builtin_marker,            _, X, (_, _, _, _, _, _, _, _, _, _), 43)\
  F(CallData,                call_data,                 X, X, (_, _, _, _, _, _, _, _, _, _), 76)\
  F(CallLiteralArgumentAst,  call_literal_argument_ast, X, _, (_, _, X, _, _, _, _, _, _, _), 80)\
  F(CallLiteralAst,          call_literal_ast,          X, _, (_, _, X, _, _, X, _, _, _, _), 79)\
  F(CallTags,                call_tags,                 X, _, (_, X, _, _, _, _, X, _, _, _), 66)\
  ...
  F(ExportedService,         exported_service,          _, X, (_, _, _, _, _, _, _, _, X, _), 91)\
  ...
  F(FreezeCheat,             freeze_cheat,              _, _, (_, _, _, _, _, _, _, X, _, _), 77)\
  F(Function,                function,                  X, X, (_, _, _, _, _, _, _, _, _, _), 56)\
  F(Guard,                   guard,                     X, _, (_, _, _, _, _, _, _, _, _, _), 30)\
  F(GuardAst,                guard_ast,                 X, X, (_, _, X, _, _, _, _, _, _, _), 38)\
  F(HardField,               hard_field,                _, X, (_, _, _, _, _, _, _, _, _, _), 31)\
  F(HashOracle,              hash_oracle,               X, X, (_, _, _, _, _, _, X, _, _, _), 87)\
  F(HashSource,              hash_source,               _, X, (_, _, _, X, _, _, _, _, _, _), 86)\
  F(Identifier,              identifier,                X, _, (X, X, X, _, _, _, _, _, _, _), 27)\
  F(IdHashMap,               id_hash_map,               X, X, (_, _, _, _, X, _, X, _, _, X), 24)\
  ...
  F(LambdaAst,               lambda_ast,                X, X, (_, _, X, _, _, X, _, _, _, _), 69)\
  F(Library,                 library,                   _, _, (_, _, X, _, _, _, _, _, _, _), 37)\
  F(LiteralAst,              literal_ast,               X, X, (_, _, X, _, _, X, _, _, _, _), 40)\
  F(LocalDeclarationAst,     local_declaration_ast,     X, X, (_, _, X, _, _, X, _, _, _, _), 20)\
  F(LocalVariableAst,        local_variable_ast,        X, X, (_, _, X, _, _, X, _, _, _, _), 12)\
  F(Method,                  method,                    X, _, (_, _, _, _, _, _, _, _, _, _), 59)\
  F(MethodAst,               method_ast,                X, X, (_, _, X, _, _, _, _, _, _, _), 29)\
  F(MethodDeclarationAst,    method_declaration_ast,    X, _, (_, _, X, _, _, _, _, _, _, _), 18)\
  F(Methodspace,             methodspace,               X, _, (_, _, _, _, _, _, X, _, _, _),  1)\
  F(Module,                  module,                    X, _, (_, _, _, _, _, _, X, _, _, _), 62)\
  F(ModuleFragment,          module_fragment,           X, _, (_, _, _, _, _, _, X, _, _, _), 16)\
  F(ModuleFragmentPrivate,   module_fragment_private,   X, X, (_, _, _, _, _, _, _, _, _, _), 39)\
  F(ModuleLoader,            module_loader,             _, _, (_, _, _, _, _, _, _, _, _, _), 13)\
  F(MutableRoots,            mutable_roots,             X, _, (_, _, _, _, _, _, X, _, _, _), 41)\
  F(Namespace,               namespace,                 X, _, (_, _, _, _, _, _, X, _, _, _), 28)\
  ...
  F(ReifiedArguments,        reified_arguments,         _, X, (_, _, _, _, _, _, _, _, _, X), 14)\
  F(Roots,                   roots,                     X, _, (_, _, _, _, _, _, X, _, _, _),  2)\
  F(Seed,                    seed,                      _, _, (_, _, X, _, _, _, _, _, _, _), 15)\
  F(SequenceAst,             sequence_ast,              X, X, (_, _, X, _, _, X, _, _, _, _), 35)\
  F(SignalAst,               signal_ast,                X, X, (_, _, X, _, _, X, _, _, _, _), 48)\
  F(SignalHandlerAst,        signal_handler_ast,        X, X, (_, _, X, _, _, X, _, _, _, _), 75)\
  F(Signature,               signature,                 X, _, (_, _, _, _, _, _, X, _, _, _), 53)\
  F(SignatureAst,            signature_ast,             X, X, (_, _, X, _, _, _, _, _, _, _), 19)\
  F(SignatureMap,            signature_map,             X, _, (_, _, _, _, _, _, X, _, _, _), 45)\
  F(SoftField,               soft_field,                _, X, (_, _, _, _, _, _, _, _, _, _), 89)\
  F(Stack,                   stack,                     _, _, (_, _, _, _, _, _, _, _, _, _), 71)\
  F(StackPiece,              stack_piece,               _, _, (_, _, _, X, _, _, _, _, _, _), 58)\
  F(SymbolAst,               symbol_ast,                X, X, (_, _, X, _, _, _, _, _, _, _), 33)\
  F(Task,                    task,                      _, _, (_, _, _, _, _, _, _, _, _, _), 82)\
  ...
  F(Utf8,                    utf8,                      _, X, (X, X, _, X, _, _, _, _, _, X), 57)\
  F(VariableAssignmentAst,   variable_assignment_ast,   X, _, (_, _, X, _, _, X, _, _, _, _), 22)\
  F(VoidP,                   void_p,                    _, _, (_, _, _, X, _, _, _, _, _, _), 26)\
  F(WithEscapeAst,           with_escape_ast,           X, _, (_, _, X, _, _, X, _, _, _, _), 23)
...
// totally worth it.
#define mfCm(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) CM
#define mfId(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) ID
#define mfPi(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) PI
#define mfNl(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) NL
#define mfFu(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) FU
#define mfEm(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) EM
#define mfOw(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) OW
#define mfFz(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) FZ
#define mfFl(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) FL
#define mfSo(CM, ID, PI, NL, FU, EM, OW, FZ, FL, SO) SO
