...
///   - _Pi_: can this be read from plankton?
///   - _Po_: can this be converted to plankton?
...
#define ENUM_OTHER_HEAP_OBJECT_FAMILIES(F)                                     \
  ...
  F(Ambience,                ambience,                  _, _, (_, _, _, _, X, _, _, _, _, _, _), 65)\
  F(ArgumentAst,             argument_ast,              X, X, (_, _, X, _, _, _, _, _, _, _, _),  9)\
  F(ArgumentMapTrie,         argument_map_trie,         X, _, (_, _, _, _, _, _, _, X, _, _, _),  7)\
  F(Array,                   array,                     X, X, (_, X, _, X, X, _, _, _, _, _, X), 11)\
  F(ArrayAst,                array_ast,                 X, X, (_, _, X, _, _, _, X, _, _, _, _), 10)\
  F(ArrayBuffer,             array_buffer,              X, X, (_, _, _, _, _, _, _, X, _, _, _), 34)\
  F(AsciiStringView,         ascii_string_view,         _, X, (_, _, _, _, _, _, _, _, _, _, _), 85)\
  F(Backtrace,               backtrace,                 _, X, (_, _, _, _, _, _, _, _, _, _, _), 54)\
  F(BacktraceEntry,          backtrace_entry,           _, _, (_, _, _, _, _, _, _, _, _, _, _), 64)\
  F(Blob,                    blob,                      _, X, (_, _, _, _, X, _, _, _, _, _, _), 63)\
  F(Block,                   block,                     X, X, (_, _, _, _, _, _, _, _, _, _, _), 73)\
  F(BlockAst,                block_ast,                 X, X, (_, _, X, _, _, _, X, _, _, _, _), 72)\
  F(BuiltinImplementation,   builtin_implementation,    X, _, (_, _, _, _, _, _, _, _, _, _, _),  6)\
  F(BuiltinMarker,           builtin_marker,            _, X, (_, _, _, _, _, _, _, _, _, _, _), 43)\
  F(CallData,                call_data,                 X, X, (_, _, _, _, _, _, _, _, _, _, _), 76)\
  F(CallLiteralArgumentAst,  call_literal_argument_ast, X, _, (_, _, X, _, _, _, _, _, _, _, _), 80)\
  F(CallLiteralAst,          call_literal_ast,          X, _, (_, _, X, _, _, _, X, _, _, _, _), 79)\
  F(CallTags,                call_tags,                 X, _, (_, X, _, _, _, _, _, X, _, _, _), 66)\
  F(CObject,                 c_object,                  _, X, (_, _, _, _, X, _, _, _, _, _, _), 67)\
  F(CodeBlock,               code_block,                X, _, (_, _, _, _, _, _, _, X, _, _, _), 49)\
  F(CurrentModuleAst,        current_module_ast,        _, _, (_, _, X, _, _, _, X, _, _, _, _), 61)\
  F(DecimalFraction,         decimal_fraction,          _, _, (_, _, X, _, _, _, _, _, _, _, _), 25)\
  F(EnsureAst,               ensure_ast,                X, X, (_, _, X, _, _, _, X, _, _, _, _), 74)\
  F(Escape,                  escape,                    _, X, (_, _, _, _, _, _, _, _, _, _, _), 50)\
  F(ExportedService,         exported_service,          _, X, (_, _, _, _, _, _, _, _, _, X, _), 91)\
  F(Factory,                 factory,                   _, _, (_, _, _, _, _, _, _, _, _, _, _),  5)\
  F(FifoBuffer,              fifo_buffer,               _, _, (_, _, _, _, _, _, _, _, _, _, _), 81)\
  F(ForeignService,          foreign_service,           _, X, (_, _, _, _, _, _, _, _, _, _, _), 90)\
  F(FreezeCheat,             freeze_cheat,              _, _, (_, _, _, _, _, _, _, _, X, _, _), 77)\
  F(Function,                function,                  X, X, (_, _, _, _, _, _, _, _, _, _, _), 56)\
  F(Guard,                   guard,                     X, _, (_, _, _, _, _, _, _, _, _, _, _), 30)\
  F(GuardAst,                guard_ast,                 X, X, (_, _, X, _, _, _, _, _, _, _, _), 38)\
  F(HardField,               hard_field,                _, X, (_, _, _, _, _, _, _, _, _, _, _), 31)\
  F(HashOracle,              hash_oracle,               X, X, (_, _, _, _, _, _, _, X, _, _, _), 87)\
  F(HashSource,              hash_source,               _, X, (_, _, _, _, X, _, _, _, _, _, _), 86)\
  F(Identifier,              identifier,                X, _, (X, X, X, _, _, _, _, _, _, _, _), 27)\
  F(IdHashMap,               id_hash_map,               X, X, (_, _, _, _, _, X, _, X, _, _, X), 24)\
  F(IncomingRequestThunk,    incoming_request_thunk,    _, X, (_, _, _, _, _, _, _, _, _, _, _), 92)\
  F(Instance,                instance,                  _, X, (_, _, X, _, _, _, _, X, _, _, X), 60)\
  F(InstanceManager,         instance_manager,          _, X, (_, _, _, _, _, _, _, _, _, _, _),  3)\
  F(InvocationAst,           invocation_ast,            X, X, (_, _, X, _, _, _, X, _, _, _, _),  4)\
  F(IsDeclarationAst,        is_declaration_ast,        X, _, (_, _, X, _, _, _, _, _, _, _, _), 21)\
  F(Lambda,                  lambda,                    X, X, (_, _, _, _, _, _, _, X, _, _, _), 47)\
  F(LambdaAst,               lambda_ast,                X, X, (_, _, X, _, _, _, X, _, _, _, _), 69)\
  F(Library,                 library,                   _, _, (_, _, X, _, _, _, _, _, _, _, _), 37)\
  F(LiteralAst,              literal_ast,               X, X, (_, _, X, _, _, _, X, _, _, _, _), 40)\
  F(LocalDeclarationAst,     local_declaration_ast,     X, X, (_, _, X, _, _, _, X, _, _, _, _), 20)\
  F(LocalVariableAst,        local_variable_ast,        X, X, (_, _, X, _, _, _, X, _, _, _, _), 12)\
  F(Method,                  method,                    X, _, (_, _, _, _, _, _, _, _, _, _, _), 59)\
  F(MethodAst,               method_ast,                X, X, (_, _, X, _, _, _, _, _, _, _, _), 29)\
  F(MethodDeclarationAst,    method_declaration_ast,    X, _, (_, _, X, _, _, _, _, _, _, _, _), 18)\
  F(Methodspace,             methodspace,               X, _, (_, _, _, _, _, _, _, X, _, _, _),  1)\
  F(Module,                  module,                    X, _, (_, _, _, _, _, _, _, X, _, _, _), 62)\
  F(ModuleFragment,          module_fragment,           X, _, (_, _, _, _, _, _, _, X, _, _, _), 16)\
  F(ModuleFragmentPrivate,   module_fragment_private,   X, X, (_, _, _, _, _, _, _, _, _, _, _), 39)\
  F(ModuleLoader,            module_loader,             _, _, (_, _, _, _, _, _, _, _, _, _, _), 13)\
  F(MutableRoots,            mutable_roots,             X, _, (_, _, _, _, _, _, _, X, _, _, _), 41)\
  F(Namespace,               namespace,                 X, _, (_, _, _, _, _, _, _, X, _, _, _), 28)\
  F(NamespaceDeclarationAst, namespace_declaration_ast, X, _, (_, _, X, _, _, _, _, _, _, _, _), 44)\
  F(NamespaceVariableAst,    namespace_variable_ast,    X, X, (_, _, X, _, _, _, X, _, _, _, _), 42)\
  F(Operation,               operation,                 X, X, (_, X, X, _, _, _, _, _, _, _, X), 46)\
  F(Parameter,               parameter,                 X, _, (_, _, _, _, _, _, _, _, _, _, _), 51)\
  F(ParameterAst,            parameter_ast,             X, X, (_, _, X, _, _, _, _, _, _, _, _),  8)\
  F(Path,                    path,                      X, X, (X, X, X, _, _, _, _, _, _, _, _), 36)\
  F(Process,                 process,                   _, _, (_, _, _, _, _, _, _, _, _, X, _), 83)\
  F(ProgramAst,              program_ast,               X, _, (_, _, X, _, _, _, _, _, _, _, _), 17)\
  F(Promise,                 promise,                   _, X, (_, _, _, _, _, _, _, _, _, _, _), 78)\
  F(Reference,               reference,                 X, _, (_, _, _, _, _, _, _, _, _, _, _), 68)\
  F(ReifiedArguments,        reified_arguments,         _, X, (_, _, _, _, _, _, _, _, _, _, X), 14)\
  F(Roots,                   roots,                     X, _, (_, _, _, _, _, _, _, X, _, _, _),  2)\
  F(Seed,                    seed,                      _, _, (_, _, X, _, _, _, _, _, _, _, _), 15)\
  F(SequenceAst,             sequence_ast,              X, X, (_, _, X, _, _, _, X, _, _, _, _), 35)\
  F(SignalAst,               signal_ast,                X, X, (_, _, X, _, _, _, X, _, _, _, _), 48)\
  F(SignalHandlerAst,        signal_handler_ast,        X, X, (_, _, X, _, _, _, X, _, _, _, _), 75)\
  F(Signature,               signature,                 X, _, (_, _, _, _, _, _, _, X, _, _, _), 53)\
  F(SignatureAst,            signature_ast,             X, X, (_, _, X, _, _, _, _, _, _, _, _), 19)\
  F(SignatureMap,            signature_map,             X, _, (_, _, _, _, _, _, _, X, _, _, _), 45)\
  F(SoftField,               soft_field,                _, X, (_, _, _, _, _, _, _, _, _, _, _), 89)\
  F(Stack,                   stack,                     _, _, (_, _, _, _, _, _, _, _, _, _, _), 71)\
  F(StackPiece,              stack_piece,               _, _, (_, _, _, _, X, _, _, _, _, _, _), 58)\
  F(SymbolAst,               symbol_ast,                X, X, (_, _, X, _, _, _, _, _, _, _, _), 33)\
  F(Task,                    task,                      _, _, (_, _, _, _, _, _, _, _, _, _, _), 82)\
  F(Type,                    type,                      X, X, (_, _, X, _, _, _, _, _, _, _, _), 32)\
  F(UnboundModule,           unbound_module,            _, _, (_, _, X, _, _, _, _, _, _, _, _), 55)\
  F(UnboundModuleFragment,   unbound_module_fragment,   _, _, (_, _, X, _, _, _, _, _, _, _, _), 52)\
  F(Utf8,                    utf8,                      _, X, (X, X, _, X, X, _, _, _, _, _, X), 57)\
  F(VariableAssignmentAst,   variable_assignment_ast,   X, _, (_, _, X, _, _, _, X, _, _, _, _), 22)\
  F(VoidP,                   void_p,                    _, _, (_, _, _, _, X, _, _, _, _, _, _), 26)\
  F(WithEscapeAst,           with_escape_ast,           X, _, (_, _, X, _, _, _, X, _, _, _, _), 23)
...
// totally worth it.
#define mfCm(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) CM
#define mfId(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) ID
#define mfPi(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) PI
#define mfPo(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) PO
#define mfNl(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) NL
#define mfFu(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) FU
#define mfEm(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) EM
#define mfOw(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) OW
#define mfFz(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) FZ
#define mfFl(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) FL
#define mfSo(CM, ID, PI, PO, NL, FU, EM, OW, FZ, FL, SO) SO
...
///   - _Cm_: do the values support ordered comparison?
///   - _Po_: can values be converted to plankton?
///   - _Sr_: is this type exposed to the surface language?
///

// Accessors for the minor phylum properties. See the accessors for minor family
// properties for details.
#define mpCm(CM, PO) CM
#define mpPo(CM, PO) PO

//  CamelName                underscore_name            Sr (Cm Po)   N
#define ENUM_CUSTOM_TAGGED_PHYLUMS(F)                                          \
  F(AsciiCharacter,          ascii_character,           X, (X, _),   0)        \
  F(Boolean,                 boolean,                   X, (X, X),   1)        \
  F(FlagSet,                 flag_set,                  _, (_, _),   2)        \
  F(Float32,                 float_32,                  X, (X, _),   3)        \
  F(Nothing,                 nothing,                   _, (_, _),   4)        \
  F(Null,                    null,                      X, (_, X),   5)        \
  F(PromiseState,            promise_state,             _, (_, _),   6)        \
  F(Relation,                relation,                  _, (_, _),   7)        \
  F(Score,                   score,                     _, (X, _),   8)        \
